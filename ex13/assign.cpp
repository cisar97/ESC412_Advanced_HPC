...
// GPU requires manual transpose and transposed order
static_assert((TRANSPOSED_ORDER&&MANUAL_TRANSPOSE) || !USE_GPU);
...
int main(int argc, char *argv[]) {
...
#ifdef USE_GPU
    cudaHostRegister(slab_data,local_n0 * nGrid * 2*k_nz * sizeof(float),cudaHostRegisterPortable);
#endif
...
#if USE_GPU
    auto [plan_2d,workSize2D] = gpu_make_plan_2D(nGrid);
    auto [plan_1d,workSize1D] = gpu_make_plan_1D(nGrid);

    Array<cudaStream_t,1> streams(3);
    Array<void*,1> gpu_slab(streams.size());
    Array<void*,1> gpu_work(streams.size());

    for(auto i=0; i<streams.size(); ++i) {
        cudaStreamCreate(&streams(i));
        gpu_slab(i) = gpu_allocate_slab(nGrid);
        gpu_work(i) = gpu_allocate(std::max(workSize2D,workSize1D));
    }
#else
    ...
    // Single rank. Do not use MPI to reduce.
    if (nrank==1) {
        assert(local_0_start==0);
        assert(local_n0==nGrid);
        Array<float,3> dst = ghost(Range(0,iOrder-2),Range::all(),Range::all());
        Array<float,3> src = ghost(Range(nGrid,nGrid+iOrder-2),Range::all(),Range::all());
        dst += src;
    }
    // Odd number of ranks. Multiple phases are required
    else if (nrank&1) {
        MPI_Comm dummycomm, comm1, comm2;
        if (irank==nrank-1) MPI_Comm_split(MPI_COMM_WORLD, MPI_UNDEFINED, 0, &dummycomm);
        else MPI_Comm_split(MPI_COMM_WORLD, irank/2, 0, &comm1);
        if (irank==0) MPI_Comm_split(MPI_COMM_WORLD, MPI_UNDEFINED, 0, &dummycomm);
        else MPI_Comm_split(MPI_COMM_WORLD, (irank+1) / 2, irank==0 ? nrank : irank, &comm2);

        if (irank==nrank-1) MPI_Comm_split(MPI_COMM_WORLD, 0, 0, &comm1);
        else if (irank==0)  MPI_Comm_split(MPI_COMM_WORLD, 0, 1, &comm2);
        else                MPI_Comm_split(MPI_COMM_WORLD, MPI_UNDEFINED, 0, &dummycomm);

        if (irank&1) {
            MPI_Reduce(MPI_IN_PLACE,&ghost(int(local_0_start),0,0),(nGrid+2)*nGrid*(iOrder-1),MPI_FLOAT,MPI_SUM,1,comm1);
            MPI_Reduce(&ghost(int(local_0_start+local_n0),0,0),nullptr,(nGrid+2)*nGrid*(iOrder-1),MPI_FLOAT,MPI_SUM,1,comm2);
        }
        else {
            if (irank!=nrank-1) MPI_Reduce(&ghost(int(local_0_start+local_n0),0,0),nullptr,(nGrid+2)*nGrid*(iOrder-1),MPI_FLOAT,MPI_SUM,1,comm1);
            if (irank!=0)       MPI_Reduce(MPI_IN_PLACE,&ghost(int(local_0_start),0,0),(nGrid+2)*nGrid*(iOrder-1),MPI_FLOAT,MPI_SUM,1,comm2);
        }
        if (irank==nrank-1) MPI_Reduce(&ghost(int(local_0_start+local_n0),0,0),nullptr,(nGrid+2)*nGrid*(iOrder-1),MPI_FLOAT,MPI_SUM,1,comm1);
        else if (irank==0)  MPI_Reduce(MPI_IN_PLACE,&ghost(int(local_0_start),0,0),(nGrid+2)*nGrid*(iOrder-1),MPI_FLOAT,MPI_SUM,1,comm2);
        MPI_Comm_free(&comm1);
        MPI_Comm_free(&comm2);
    }
...
#if USE_GPU
	auto istream = i % streams.size();
	Array<float,2> slice = slab(int(local_0_start+i),Range::all(),Range::all());
	gpu_fft_2D_R2C(slice,gpu_slab(istream),plan_2d,streams(istream),gpu_work(istream),nGrid,diRhoBar);
#else
...
#if USE_GPU
	auto istream = i % streams.size();
    	Array<std::complex<float>,2> slice = kslab(Range::all(),int(local_1_start+i),Range::all());
    	gpu_fft_1D_C2C(slice,gpu_slab(istream),plan_1d,streams(istream),gpu_work(istream));
#else
